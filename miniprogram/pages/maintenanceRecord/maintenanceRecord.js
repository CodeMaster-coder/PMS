Page({
  ...
  inputMaintaincePosition(e){
    this.setData({
      maintaincePosition:e.detail.value
    })
...
if(that.data.maintainers == ''){
    ...
    else if(that.data.maintaincePosition == ''){
      wx.showToast({
        title: '请输入维保位置',
        icon:'error',
        duration:3000
        ...
        success:function(res){    
          if (res.confirm) {
            wx.request({
              ...
              data:{
                code:'maintainceInfoInsert',
                maintaincePosition:that.data.maintaincePosition,
